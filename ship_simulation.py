...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                             ...
                             QCheckBox, QComboBox, QTextBrowser, QFileDialog)
...
class Ship:
    ...
    def get_log_row(self, time_s):
        """Return CSV row: time, x_m, y_m, course, speed, rudder, rpm"""
        return [
            f"{time_s:.2f}",
            f"{self.x:.2f}",
            f"{self.y:.2f}",
...
class MainWindow(QMainWindow):
    ...
    def save_task(self):
        """Save current vessel configuration to a task file"""
        if len(self.ships) == 0:
    ...
    def load_task(self):
        """Load vessel configuration from a task file"""
    ...
    def show_help(self):
        ...
        <li><b>Control vessel:</b> <b>RMB on vessel icon</b>. Opens panel with rudder/RPM and LLM toggle.</li>
        </ul>
        <h3>Tasks:</h3>
        <ul>
        ...
        <li>Enable LLM (via menu or RMB on vessel).</li>
    ...
    def simulation_step(self):
        if self.running:
            ...
            if (llm_ships and
                    (self.simulation_time - self.last_llm_update >= self.llm_update_interval)):
                collision_data = self.collect_collision_data()
                if collision_data:
                    coordinator = self.get_or_create_coordinator()
                    commands = coordinator.get_coordinated_commands(
                        self.ships, collision_data)
                    coordinator.apply_commands(self.ships, commands)
